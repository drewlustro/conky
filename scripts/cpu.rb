...
cpu_freqs.each_with_index do |freq, index|
  core_index = index + 1
  string << "${color0}CPU Core #{core_index}: $color #{freq}MHz ${color0} Load: $color${cpu cpu#{core_index}}% $color0${goto 460}Temp: $color${exec sensors | grep 'Core #{index}' | cut -c18-19}°C\n"
